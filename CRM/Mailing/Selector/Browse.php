...
LEFT JOIN  civicrm_contact scheduledContact ON ( $mailing.scheduled_id = scheduledContact.id )
  ...
  public function whereClause(&$params, $sortBy = TRUE) {
    ...
    $isJourney=$this->_parent->get('is_journey');
    ...
    // Show journey not checked or false --> select only not journey emails
    // Else: show all --> no additional clause required
    if (!isset($isJourney) || !$isJourney) {
      $clauses[] = "(civicrm_mailing.is_journey IS NULL OR civicrm_mailing.is_journey = 0)";
    }
